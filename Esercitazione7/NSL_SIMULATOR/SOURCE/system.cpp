...
int System :: gibbs(int i) {
	double delta_E, p;
    
  delta_E = 2*_J*(_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() +_H );
  p = 1./(1+exp(-_beta*delta_E));
    
  if(_rnd.Rannyu() <= p) return 1;
  else return -1;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8./9*M_PI*_rho*(1./pow(_r_cut,9)-3./pow(_r_cut,3));
        ...
        index_property++;
        	if (!_measure_kenergy) {
        		cout << endl << "The measure of the kinetic energy has been added since it is needed to compute the temperature." << endl;
        		ofstream coutk("../OUTPUT/kinetic_energy.dat");
        		coutk << "#     BLOCK:   ACTUAL_KE:    KE_AVE:      ERROR:" << endl;
        		coutk.close();
       		_nprop++;
        ...
        _ptail = 32*M_PI*_rho*(1./(9.*pow(_r_cut,9))-1./(6.*pow(_r_cut,3)));
        		if (!_measure_temp) {
        		cout << endl << "The measure of the temperature has been added since it is needed to compute the pressure." << endl;
        		ofstream coutte("../OUTPUT/temperature.dat");
        		coutte << "#     BLOCK:   ACTUAL_T:     T_AVE:       ERROR:" << endl;
        		coutte.close();
       		_nprop++;
        ...
        coutgr.close();
        ofstream coutgrc("../OUTPUT/gofr_complete.dat");
        coutgrc << "#        BLOCK:          R:        ACTUAL_GOFR:          GOFR_AVE:           ERROR: " << endl;
        coutgrc.close();
        input>>_n_bins;
        _nprop+=_n_bins;
        ...
        index_property++;
        if (!_measure_tenergy) {
        	cout << endl << "The measure of the total energy has been added since it is needed to compute the specific heat." << endl;
        	ofstream coutt("../OUTPUT/total_energy.dat");
        	coutt << "#     BLOCK:   ACTUAL_TE:    TE_AVE:      ERROR:" << endl;
        	coutt.close();
        	_nprop++;
...
void System :: initialize_different_T() {
	string property;
	ifstream input("../INPUT/properties.dat");
   ...
   if (input.is_open()){
    while ( !input.eof() ){
      input >> property;
        ...
        ofstream coutt("../OUTPUT/total_energy_final.dat");
        coutt << "#     T:   TE_AVE:      ERROR:" << endl;
        coutt.close();
      } else if( property == "MAGNETIZATION" ){
        ofstream coutpr("../OUTPUT/magnetization_final.dat");
        coutpr << "#     T:     M_AVE:       ERROR:" << endl;
        coutpr.close();
      } 
      	else if( property == "SPECIFIC_HEAT" ){
        ofstream coutpr("../OUTPUT/specific_heat_final.dat");
        coutpr << "#     T:    CV_AVE:      ERROR:" << endl;
        coutpr.close();
      } else if( property == "SUSCEPTIBILITY" ){
        ofstream coutpr("../OUTPUT/susceptibility_final.dat");
        coutpr << "#     T:   X_AVE:       ERROR:" << endl;
        coutpr.close();
      }
    }
    input.close();
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        // GOFR ////////////////////////////////////////////////////////////////
        if (dr <= _halfside.min() && _measure_gofr) {
        _measurement(_index_gofr + static_cast<int>(dr/_bin_size)) += 2;
        }
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
	if (_measure_pressure) {
		pressure_temp = 16./_volume*pressure_temp;
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
	if (_measure_cv and _measure_tenergy) {
			double energy_cv = 0.0;
			double s_i, s_j;
      for (int i=0; i<_npart; i++){
        s_i = double(_particle(i).getspin());
        s_j = double(_particle(this->pbc(i+1)).getspin());
        energy_cv += - _J * s_i * s_j - 0.5 * _H * (s_i + s_j);
      }
      
      cv_temp = energy_cv*energy_cv;
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
	if (_measure_chi) {
		...
		_measurement(_index_chi) = _beta*chi*chi/_npart;
	}
...
void System :: averages(int blk){
  ...
  if (_measure_pressure) _average(_index_pressure) += (_ptail*_npart/_volume+_rho*_measurement(_index_temp));
  if (_measure_cv)  _average(_index_cv) = _beta*_beta/_npart*( _average(_index_cv) - (_npart*_average(_index_tenergy))*(_npart*_average(_index_tenergy))); 
  if (_measure_gofr) {
  	for (int i=0; i<_n_bins; i++) {
    	_average(_index_gofr+i) /= (_rho*_npart*4./3.*M_PI*( pow(((i+1)*_bin_size),3) - pow((i*_bin_size),3) ) );
    }
   }
  _global_av  += _average;
  ...
  if (_measure_gofr){
    coutf_gofr.open("../OUTPUT/gofr_complete.dat",ios::app);
    ...
    for (int i=0; i<_n_bins; i++) {
    	...
    	coutf_gofr << setw(12) << blk 
    				  << setw(18) << scientific << i*_bin_size 
          	     << setw(18)  << scientific << average
          	     << setw(18) << scientific << sum_average/double(blk)
          	     << setw(18) << scientific << this->error(sum_average, sum_ave2, blk) << endl;
...
void System :: choose_delta (System &SYS, double accepted_fraction) {

	 int total_delta_trials_more = 0;
    int total_delta_trials_less = 0;
    double delta_trial = _delta;
    double resize = _delta / 100.0;
    ...
    do {    	
    	 ...
    	 if ((total_delta_trials_more + total_delta_trials_less) % 2 == 0) {
            _delta= delta_trial + resize * total_delta_trials_more;
            total_delta_trials_more++;
        } else {
            _delta = delta_trial - resize * total_delta_trials_less;
            total_delta_trials_less++;
        }
      
      for (int i=0; i<5; i++) SYS.step();
    	
    	// If the delta trials are exhausted, exit(0)
      if (total_delta_trials_less == 100) {
          cout << "Couldn't find a delta for the uniform distribution with the requested acceptance. Try another initial guess." << endl;
          exit(0);
      }
      
      } while (!((accepted_fraction - 0.05) <= (static_cast<double>(_naccepted) / static_cast<double>(_nattempts)) && (static_cast<double>(_naccepted) / static_cast<double>(_nattempts)) <= (accepted_fraction + 0.05)));
      
      cout << endl << "Delta used: " << _delta;
      cout << endl << "Acceptance ratio: " << static_cast<double>(_naccepted)/static_cast<double>(_nattempts);
