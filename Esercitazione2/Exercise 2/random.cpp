...
double Random :: Exp (double lambda) {
	// This function generates a random number from an exponential distribution with given decay rate lambda (p(x)=1/lambda*exp(-lambda*x)
	double y=Rannyu();
	return -1./lambda*log(1-y);
}

double Random :: CauchyLorentz (double mu, double Gamma) {
	// This function generates a random number from a Cauchy-Lorentz distribution with given location parameter mu and given scale parameter Gamma
	double y=Rannyu();
	return (mu+Gamma*tan(M_PI*(y-0.5)));
}

double Random :: my_prob(void){
   // This function generates a random number in the range [min, max)
