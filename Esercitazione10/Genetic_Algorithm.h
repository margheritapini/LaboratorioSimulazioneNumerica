...
struct individual {
	...
	individual(int ncity) {
		...
		for (int i=0; i<ncity; i++) _chromosome(i) = i+1;
	}
	...
	void permutation_cityorder (individual other, int n_permutations, Random& _rnd) {
		...
		//for (int i=0; i<50; i++) j1=_rnd.Rannyu(1, _ncity);
		
		for (int i=0; i<n_permutations; i++) {
			j1 = abs(_rnd.Rannyu(1, _ncity));
			j2 = abs(_rnd.Rannyu(1, _ncity));
...
public: // Function declarations
	...
	int* get_best_chromosome();
	void set_best_chromosome(int*);
	void initialize_output_file(int rank);
	vector<int>  choose_couples (int size);
	void save_path(int rank);
	void measure(int generation, int rank);
